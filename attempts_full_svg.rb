...
svg1 = create_svg('attempts_full_levels.svg', "Level attempts (Total: #{total_l})", $limit_l, 'Attempts', 'Level', data_l.map{ |s| s[0] }, data_l.map{ |s| s[1] }.reverse)
svg2 = create_svg('attempts_full_episodes.svg', "Episode attempts (Total: #{total_e})", $limit_e, 'Attempts', 'Episode', data_e.map{ |s| s[0] }, data_e.map{ |s| s[1] }.reverse)
