...
data_l = (0..n_l - 1).to_a.map{ |i|
  ...
  [_unpack(file[r][12..15]) + _unpack(file[r][16..19]), ids_l[i]]
}.sort_by{ |s| -s[0] }
...
data_e = (0..n_e - 1).to_a.map{ |i|
  ...
  [_unpack(file[r][12..15]) + _unpack(file[r][16..19]), ids_e[i]]
}.sort_by{ |s| -s[0] }
...
svg1 = create_svg('successes_levels.svg', "Level successes (Total: #{total_l})", 'Successes', 'Levels', data_l.map{ |s| s[0] }.reverse, data_l.map{ |s| s[1] }.reverse)
svg2 = create_svg('successes_episodes.svg', "Episode successes (Total: #{total_e})", 'Successes', 'Levels', data_e.map{ |s| s[0] }.reverse, data_e.map{ |s| s[1] }.reverse)
