...
data_l = (0..n_l - 1).to_a.map{ |i|
  ...
  [_unpack(file[r][4..7]), _unpack(file[r][12..15]) + _unpack(file[r][16..19]), ids_l[i]]
}.sort_by{ |s| -s[0] }
...
data_e = (0..n_e - 1).to_a.map{ |i|
  ...
  [_unpack(file[r][4..7]), _unpack(file[r][12..15]) + _unpack(file[r][16..19]), ids_e[i]]
}.sort_by{ |s| -s[0] }
total_e = data_e.sum{ |s| s[0] }
data_e = data_e.take(limit)

# Bar plot (SVG)
def create_svg(filename, title, x, y, data1, data2, labels)
  options = {
    :width             => 800,
    :height            => 500,
    ...
    :scale_divisions   => (data1.max.to_f / 6).round,
    :scale_integers    => true,
    :no_css            => true
  }
  g = SVG::Graph::BarHorizontal.new(options)
  g.add_data({:data => data1, :title => "Attempts"})
  g.add_data({:data => data2, :title => "Successes"})
  File.open(filename, 'w') {|f| f.write(g.burn_svg_only)}
end

svg1 = create_svg('attsuc_levels.svg', "Level attempts and successes (Total: #{total_l})", 'Attempts', 'Levels', data_l.map{ |s| s[0] }.reverse, data_l.map{ |s| s[1] }.reverse, data_l.map{ |s| s[2] }.reverse)
svg2 = create_svg('attsuc_episodes.svg', "Episode attempts and successes (Total: #{total_e})", 'Attempts', 'Levels', data_e.map{ |s| s[0] }.reverse, data_e.map{ |s| s[1] }.reverse, data_e.map{ |s| s[2] }.reverse)
